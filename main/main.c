...
//uint8_t voice_desc[2] = {0x01,0x00};

//static esp_attr_value_t voice_desc_val = {
	//.attr_max_len   = 22,
	//.attr_len		= sizeof(voice_desc),
	//.attr_value     = voice_desc,
//};
static esp_gatt_if_t gatts_if_gol;
static uint16_t connection_id_gol;
static uint16_t char_handle_gol;

static uint8_t char1_str[8];
static esp_gatt_char_prop_t a_property = 0;
static esp_gatt_char_prop_t b_property = 0;
volatile int get_adc_flag = 0;
volatile int set_up = 0;
volatile uint32_t pointer = 0;
...
true, .include_txpower = true, .min_interval = 0x0004, //slave connection min interval, Time = min_interval * 1.25 msec
		.max_interval = 0x0004, //slave connection max interval, Time = max_interval * 1.25 msec
...
static esp_ble_adv_data_t scan_rsp_data = { .set_scan_rsp = true,
		.include_name = true, .include_txpower = true, .min_interval = 0x0004,
		.max_interval = 0x0004, .appearance = 0x00, .manufacturer_len = 0, //TEST_MANUFACTURER_DATA_LEN,
		.p_manufacturer_data = NULL, //&test_manufacturer[0],
		.service_data_len = 0, .p_service_data = NULL, .service_uuid_len =
				sizeof(adv_service_uuid128), .p_service_uuid =
				adv_service_uuid128, .flag = (ESP_BLE_ADV_FLAG_GEN_DISC
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event,
	...
	case ESP_GATTS_REG_EVT:
		...
		TEST_DEVICE_NAME);
		gatts_if_gol = gatts_if; //Setting the global interface value
		if (set_dev_name_ret) {
	...
	case ESP_GATTS_READ_EVT: {
		...
		rsp.attr_value.value[0] = char1_str[0];
		rsp.attr_value.value[1] = char1_str[1];
		rsp.attr_value.value[2] = char1_str[2];
		rsp.attr_value.value[3] = char1_str[3];
		rsp.attr_value.value[4] = char1_str[4];
		esp_ble_gatts_send_response(gatts_if, param->read.conn_id,
				param->read.trans_id, ESP_GATT_OK, &rsp); //Send the response
		break;
	}
	...
	case ESP_GATTS_CREATE_EVT:
		...
		esp_err_t add_char_ret = esp_ble_gatts_add_char(
				...
				ESP_GATT_PERM_READ |ESP_GATT_PERM_WRITE, a_property, &gatts_demo_char1_val,
				NULL); //Response has to be given manually by the application
		if (add_char_ret) {
	...
	case ESP_GATTS_ADD_CHAR_EVT: {
		...
		char_handle_gol = param->add_char.attr_handle;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event,
	...
	case ESP_GATTS_CONNECT_EVT:
		...
		connection_id_gol = param->connect.conn_id;
		break;
	case ESP_GATTS_CONF_EVT:
		ESP_LOGI(GATTS_TAG, "ESP_GATTS_CONF_EVT status %d attr_handle %d",
				param->conf.status, param->conf.handle);
...
//Have to manually notify the client that the data has changed via blue-tooth notifications
void read_value(void *pvParameter) {

	uint16_t val = 0;
	uint8_t resize_val = 0;
	while (1) {
	    adc1_config_width(ADC_WIDTH_BIT_12);
		adc1_config_channel_atten(ADC1_CHANNEL_5, ADC_ATTEN_DB_0);
		val = adc1_get_raw(ADC1_CHANNEL_5);
		resize_val = (val / 4096.0) * 256;
		char1_str[pointer] = resize_val;
		pointer++;
		if(pointer == 4 && set_up == 0){
			pointer = 0;
		}
		if (pointer == 4 && set_up == 1) {
			//ESP_ERROR_CHECK(esp_ble_gatts_send_indicate(gatts_if_gol,connection_id_gol,char_handle_gol,sizeof(char1_str),char1_str,false));
			pointer = 0;
			unsigned long temp = xx_time_get_time();
			static uint8_t first_temp_byte = 0;
			static uint8_t second_temp_byte = 0;
			static uint8_t third_temp_byte = 0;
			static uint8_t fourth_temp_byte = 0;
			first_temp_byte  = temp & 0x000000ff;
		    second_temp_byte = (temp >> 8) & 0x000000ff;
		    third_temp_byte  = (temp >> 16) & 0x000000ff;
		    fourth_temp_byte = temp >> 24;
		    char1_str[4] = fourth_temp_byte;
		    char1_str[5] = third_temp_byte;
		    char1_str[6] = second_temp_byte;
		    char1_str[7] = first_temp_byte;
		    ESP_ERROR_CHECK(esp_ble_gatts_send_indicate(gatts_if_gol,connection_id_gol,char_handle_gol,sizeof(char1_str),char1_str,false));
			//printf("Timestamp: %lu ",xx_time_get_time()); //Print the timestamp after sending a packet
		}
...
void app_main() {
	...
	//adc1_config_channel_atten(ADC1_CHANNEL_5, ADC_ATTEN_DB_0);
	//
	xTaskCreate(&read_value, "read_value", 2048, NULL, 2, NULL);
	/*
	 //xTaskCreate(&read_value, "read_value",2048,NULL,2,NULL);
	 ...
	 ESP_ERROR_CHECK(timer_init(TIMER_GROUP_0, TIMER_0, &mytimer)); //Init the timer
	 ESP_ERROR_CHECK(timer_set_alarm_value(TIMER_GROUP_0, TIMER_0, 250)); //Set the alarm value
	 ESP_ERROR_CHECK(timer_set_counter_value(TIMER_GROUP_0, TIMER_0, 0)); //Set the value to start counting from
	 ESP_ERROR_CHECK(timer_enable_intr(TIMER_GROUP_0, TIMER_0)); //Enable the timer interrupt
	 ESP_ERROR_CHECK(timer_isr_register(TIMER_GROUP_0, TIMER_0, timer_isr, NULL,
	 ESP_INTR_FLAG_IRAM, NULL));
	 ESP_ERROR_CHECK(timer_start(TIMER_GROUP_0, TIMER_0));     //Register the isr
